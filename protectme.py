...
Description:
  ...
  - Si clé absente, le bloc peut retomber sur comparaisons remote.
"""
...
def normalize_github_raw(url: str) -> str:
    ...
    if "github.com" in parsed.netloc:
        ...
        if len(path) > 4 and path[3] == "blob":
            user, repo, branch = path[1], path[2], path[4]; rest = "/".join(path[5:])
            return f"https://raw.githubusercontent.com/{user}/{repo}/{branch}/{rest}"
        m = re.match(r"^/([^/]+)/([^/]+)/refs/heads/([^/]+)/(.*)$", "/".join(path))
        if m:
...
def inject(local_path: str, remote_url: str, verbose: bool=False) -> int:
    ...
    try: remote_norm = compute_normalized_bytes_from_url(remote_url)
    except Exception as e: print("[ERREUR] impossible de fetch remote:", e); return 4
    key = ensure_key()
    ...
    if si!=-1 and ei!=-1:
        ...
        except Exception: new = protector + orig
        action="inséré"
